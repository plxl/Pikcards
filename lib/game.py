...
class Game:
    ...
    def GetEmptyLanes(self, player: Player | None = None, lanes: list[Lane] = None) -> list[Lane]:
        if player is not None:
            player_index = self.players.index(player)
            return [
                lane for lane in (lanes if lanes else self.lanes)
    ...
    def GetEmptyLaneIndices(self, player: Player | None = None,
                            lanes: list[Lane] = None) -> list[int]:
        return [
            self.lanes.index(lane) for lane in self.GetEmptyLanes(player, lanes)
        ]
    ...
    def GetLaneIndicesWithAreas(self) -> list[int]:
        return [
            self.lanes.index(lane) for lane in self.GetLanesWithAreas()
        ]
    ...
    def GetCardPlayableLanes(self, player: Player, card: Card) -> list[bool]:
        ...
        ### IMPORTANT: THIS IS NOT MODULAR FOR MORE/LESS THAN 5 LANES!
    ...
    def IsValidMove(self, move: str, player: Player) -> bool:
        ...
        if not card_playable.lanes[lane_index]:
            print(f"You're unable to play '{card_playable.card.name}' in Lane {lane_index + 1}.")
            return False
    ...
    def game_loop(self):
        ...
        while True:
            ...
            for lane in self.lanes:
                lane_index = self.lanes.index(lane) + 1
                ...
                if p1_has_minion and p2_has_minion:
                    ...
                    if fs in m1.traits and not (fs in m2.traits):
                        ...
                        m2.health -= m1.attack
                        if m2.health <= 0:
                            print(f'{ls}Your {m1.name} defeated the {m2.name} before it ' \
                                'could attack!')
                            minions[1].remove(m2)
                        else:
                            m1.health -= m2.attack
                    ...
                    if m1.health <= 0 and m2.health <= 0:
                        print(f"{ls}The {m1.name} and {m2.name} defeated each " \
                    ...
                    if p2.health <= 0:
                        ...
                        print(f"{ls}Your {m1.name} dealt {m1.attack} damage to" \
                            " CPU!")
                    ...
                    if p1.health <= 0:
                        ...
                        print(f"{ls}CPU's {m2.name} dealt {m2.attack} damage to" \
                            " you...")
            ...
            self.morning_player = self.players[1 - self.players.index(self.morning_player)]
            self.round += 1
    ...
    def player_turn(self, player: Player):
        ...
        print('...')
        sleep(0.2)
        print('Place Card: xLy  | x = card#, y = lane#   | e.g. 1L3 (puts card 1 from hand in lane 3)')
        print('Board Info: ixLy | x = player#, y = lane# | e.g. i2L4 (gives info on your opponents card in lane 4)')
        print('Hand Info:  ihx  | x = card#              | e.g. ih2 (gives info on card 2 in your hand)')
        print('End Turn:   end  |')
        sleep(0.2)
        print('...')
        sleep(0.2)
